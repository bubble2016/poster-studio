...
> 相比上一版本（`v4.0.3`），本次聚焦缓存清理、仓库同步准备与静态资源版本更新。

### 清理
- 清理本地缓存与测试产物目录：`__pycache__/`、`.pytest_cache/`、`.playwright-cli/`、`output/`。
- 恢复并补充 `.gitignore` 忽略规则，避免运行数据与临时产物被误追踪（`web_data/`、`.tmp_compress/` 等）。

### 优化
- 首页移除 Google Fonts 远程依赖，改为本地系统字体栈，降低外链字体加载失败风险。
- 统一静态资源版本号为 `20260305v404`（`favicon`、`style.css`、`app.js`、`admin.js`），确保发布后前端缓存及时刷新。
## 2026-03-01（v4.0.3）

> 相比上一版本（`v4.0.2`），本次聚焦设置面板移动端体验、弹窗操作收敛、备案信息补充，以及生成链路与会话持久化增强。

### 新增
- 新增备案信息展示：
  - 首页底部加入备案号 `豫ICP备2026007593号`。
  - `用户协议`、`隐私政策` 页面底部新增备案链接，统一指向工信部备案系统。
- 新增首页首屏主题预注入：
  - 服务端根据用户主题色预生成 `primary` 系列 CSS 变量，减少首屏主题闪动。
  - 首屏支持预注入随机背景变体类，提升首次渲染完整度。
- 新增生成链路增强：
  - 生成接口返回图片文案时，支持微信内长按保存预览与复制文案分流。
  - 下载接口支持 `inline` 模式，便于站内直接预览已生成图片。
- 新增后端安全与保留策略：
  - 预览/生成接口新增严格日期校验，拒绝无效日期输入。
  - 单用户历史导出文件支持按数量上限自动清理，默认保留最近 `3` 个。

### 优化
- 优化设置弹窗标签栏：
  - 改为单行等宽标签样式，移动端不再拆成两行。
  - “全局设置”标题区与标签栏之间去除缝隙，视觉连续。
  - 标签栏自动隐藏仅保留移动端，桌面端固定显示，消除鼠标滚轮回滚顿挫。
- 优化弹窗操作语义：
  - 移除 `全局设置`、`用户登录`、`裁剪 Logo` 中与底部“取消”重复的头部“关闭”按钮。
- 优化页脚展示：
  - 去掉备案与协议链接下方虚线样式。
  - 备案号在移动端单独占一行，避免底部信息换行拥挤。
- 优化会话与草稿体验：
  - 会话有效期从 `14` 天延长到 `30` 天。
  - 访客草稿保存增加变更比对与延迟写入，减少重复写入 LocalStorage。

### 修复
- 修复设置面板标签栏隐藏时标题与内容区之间出现缝隙的问题。
- 修复桌面端滚轮触发标签栏显隐时的顿挫感，改为桌面端固定展示。
- 修复预览重复请求与部分无效日期输入未被前后端同时拦截的问题。

### 清理
- 清理设置标签栏显隐过程中的冗余滚动补偿逻辑，仅保留移动端所需最小实现。
- 更新首页静态资源版本号为 `20260301v403`（`style.css`、`app.js`），确保发布后前端缓存及时刷新。

## 2026-02-25（v4.0.2）

> 本次聚焦仓库清理与发布一致性，完成冗余目录治理、Git 同步准备与静态资源版本统一。

### 清理
- 清理本地测试产物目录：`.playwright-cli/`、`output/`。
- 补充 `.gitignore` 忽略规则，避免临时产物被重复追踪。

### 优化
- 统一静态资源版本号为 `20260225v421`（`favicon`、`style.css`、`app.js`、`admin.js`），降低缓存不一致风险。
## 2026-02-24（v4.0.1）
